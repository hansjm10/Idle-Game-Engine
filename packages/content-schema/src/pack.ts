...
const normalizeContentPack = (
  ...
  // Build set of existing resource IDs for deduplication
  const existingResourceIds = new Set(pack.resources.map((r) => r.id));

  // Auto-generate prestige count resources for prestige layers that don't have them
  const autoGeneratedPrestigeCountResources: NormalizedResource[] = [];
  for (const layer of pack.prestigeLayers) {
    const prestigeCountId = `${layer.id}-prestige-count` as ContentId;
    if (!existingResourceIds.has(prestigeCountId)) {
      // Create a synthetic prestige count resource
      const prestigeCountResource = freezeObject({
        id: prestigeCountId,
        name: normalize(
          { default: `${layer.name.default} Count`, variants: {} },
          ['resources', pack.resources.length + autoGeneratedPrestigeCountResources.length, 'name'],
        ),
        category: 'misc' as const,
        economyClassification: 'soft' as const,
        tier: 1,
        startAmount: 0,
        capacity: null,
        visible: false,
      ...
      autoGeneratedPrestigeCountResources.push(prestigeCountResource);
    }
  }
  ...
  const normalizedResources = freezeArray([
    ...
    ...autoGeneratedPrestigeCountResources,
  ]);
