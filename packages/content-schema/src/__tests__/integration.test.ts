...
import {
  ...
  prestigeLayerWithAutoGeneratedCountFixture,
...
describe('Integration: Prestige Layer Auto-Generated Resources', () => {
  it('auto-generates prestige count resource for each prestige layer', () => {
    const validator = createContentPackValidator();
    const result = validator.safeParse(prestigeLayerWithAutoGeneratedCountFixture);
    ...
    // The prestige count resource should be auto-generated
    const prestigeCountResourceId = 'prestige-test-pack.ascension-prestige-count' as ContentId;
    const prestigeCountResource = pack.lookup.resources.get(prestigeCountResourceId);

    expect(prestigeCountResource).toBeDefined();
    expect(prestigeCountResource?.id).toBe('prestige-test-pack.ascension-prestige-count');
    expect(prestigeCountResource?.category).toBe('misc');
    expect(prestigeCountResource?.startAmount).toBe(0);
    expect(prestigeCountResource?.visible).toBe(false);
    expect(prestigeCountResource?.unlocked).toBe(true);

    // Original resources should still be present
    expect(pack.lookup.resources.get('prestige-test-pack.energy' as ContentId)).toBeDefined();
    expect(pack.lookup.resources.get('prestige-test-pack.prestige-points' as ContentId)).toBeDefined();

    // Total should be 3 resources (2 original + 1 auto-generated)
    expect(pack.resources).toHaveLength(3);
  });

  it('does not duplicate prestige count resource if manually defined', () => {
    const validator = createContentPackValidator();

    // Create a fixture that already has the prestige count resource manually defined
    const fixtureWithManualCount = {
      ...prestigeLayerWithAutoGeneratedCountFixture,
      resources: [
        ...prestigeLayerWithAutoGeneratedCountFixture.resources,
        {
          id: 'prestige-test-pack.ascension-prestige-count',
          name: { default: 'Manual Prestige Count', variants: {} },
    ...
    const result = validator.safeParse(fixtureWithManualCount);
    ...
    // Should still have only 3 resources (no duplicate created)
    expect(pack.resources).toHaveLength(3);

    // The manually defined resource should be used (with custom name)
    const prestigeCountResource = pack.lookup.resources.get(
      'prestige-test-pack.ascension-prestige-count' as ContentId
    );
    expect(prestigeCountResource?.name.default).toBe('Manual Prestige Count');
  });
});
