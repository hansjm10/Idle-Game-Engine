...
describe('Integration: prestige layer status transitions', () => {
  ...
  it('works with auto-generated prestige count resource', () => {
    // Prestige count resources are now auto-generated during content pack normalization.
    // This test verifies that the coordinator works correctly without manually defining one.
    const energy = createResourceDefinition('resource.energy', {
      name: 'Energy',
      startAmount: 100, // Enough to unlock prestige
    });
    ...
    // Note: We intentionally don't define 'prestige.ascension-prestige-count'
    // because it should be auto-generated by content pack normalization
    const prestigeLayer = createPrestigeLayerDefinition('prestige.ascension', {
      name: 'Ascension',
      resetTargets: ['resource.energy'],
      unlockCondition: {
    ...
    const pack = createContentPack({
      ...
      // No prestige count resource defined - it should be auto-generated
      prestigeLayers: [prestigeLayer],
    });

    // Should NOT throw - the prestige count resource is auto-generated
    const coordinator = createProgressionCoordinator({
      content: pack,
      stepDurationMs: 100,
    });

    // Verify the prestige layer works correctly
    const quote = coordinator.prestigeEvaluator!.getPrestigeQuote('prestige.ascension');
    expect(quote).toBeDefined();
    expect(quote!.status).toBe('available');
  });
