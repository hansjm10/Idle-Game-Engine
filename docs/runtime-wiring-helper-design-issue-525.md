...
- **Finalize/apply semantics wiring**:
  ...
  - UI rate display note: when using the core `ResourceState` (supports `finalizeTick`), direct-apply production updates balances but does not populate `incomePerSecond` / `expensePerSecond` / `netPerSecond`, so `buildProgressionSnapshot(...).resources[].perSecond` and `.perTick` will remain `0` unless finalize/apply semantics (or alternative rate tracking) are used. `createGameRuntime` defaults `applyViaFinalizeTick` to `true` to avoid this; `wireGameRuntime` keeps explicit opt-in for advanced wiring.
