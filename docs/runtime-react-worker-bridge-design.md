...
3. Shell adapter normalises metadata (e.g., converts `capturedAt` to UTC) and writes the entry to the `sessions` object store within a single IndexedDB transaction. On error it surfaces telemetry through the existing `WorkerBridge` error channel.
...
2. Shell computes offline elapsed time as `max(now - capturedAt, 0)`, passes `maxElapsedMs`/`maxSteps` cap policy to core, and derives optional `resourceDeltas` if migrations supply them.
3. Shell calls `WorkerBridge.restoreSession({ state, commandQueue, elapsedMs, maxElapsedMs, maxSteps, resourceDeltas })`. The worker follows the existing restore path, emitting either `SESSION_RESTORED` or `ERROR { code: 'RESTORE_FAILED' }`.
