...
1. **Finalize**: Once systems finish queuing per-second rates (for example, `createProductionSystem({ applyViaFinalizeTick: true })`), call `resourceState.finalizeTick(context.deltaMs)` so accumulated income/expense rolls into balances and `netPerSecond` is recomputed deterministically (§5.2).
