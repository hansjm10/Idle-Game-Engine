...
- Devtools must explicitly opt in before the worker starts emitting timeline buffers. Because `postMessage()` copies data by default,[^postmessage-copy] the worker sends deltas over a dedicated `DIAGNOSTICS_UPDATE` envelope only when the overlay is listening. Regular state updates stay slim, while the diagnostics envelope can transfer typed buffers once the overlay requests high-volume samples.[^transferable-zero-copy]
